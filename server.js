...
// Resend client (HTTP-based, works fine on Render — no SMTP ports involved)
...
app.post("/api/send-estimate", upload.single("pdf"), async (req, res) => {
  try {
    ...
    // Build attachments in Resend's format (content can be a Buffer directly)
    ...
    // Send email via Resend's HTTPS API (no SMTP ports involved,
    // so this isn't affected by Render's outbound SMTP restrictions)
    const { data, error: sendError } = await resend.emails.send({
      from: `GameOn Solution <${process.env.RESEND_FROM_EMAIL}>`,
      to: [estimateData.email],
      replyTo: process.env.REPLY_TO_EMAIL || process.env.RESEND_FROM_EMAIL,
      subject: emailTemplate.subject,
      html: emailTemplate.html,
      text: emailTemplate.text,
      attachments,
